...
QWidget *BooksDelegate::createEditor(QWidget *parent0, const QStyleOptionViewItem &option, const QModelIndex &index) const 
{
   ...
   //////////// qDebug() << "### createEditor -> " << index.data(Qt::DisplayRole).toString();
   ...
   if (index.column() == 1) {
         ...
         connect(editor, SIGNAL(currentIndexChanged(int)), this, SLOT(emitCommitData())); 
         return editor;
...
void BooksDelegate::setEditorData(QWidget *editor, const QModelIndex &index)
{
    ...
    ///////////qDebug() << "### setEditorData  -> " << index.data(Qt::DisplayRole).toString();
    
    
        if (index.column() == 1) {
            QComboBox *editorax = qobject_cast<QComboBox *>(editor);
            int itindex = editorax->findText(index.data(Qt::DisplayRole).toString());
             editorax->setCurrentIndex ( itindex );
            //////////// qDebug() << "### click index  -> " << itindex;
             //////
            
        } else {
           QLineEdit *editoraxt = qobject_cast<QLineEdit *>(editor);
            editoraxt->setText(index.data(Qt::DisplayRole).toString()); 
...
{
     ///////////qDebug() << "### setModelData  -> " << index.data(Qt::DisplayRole).toString();
    
     if (index.column() == 1) {
            QComboBox *editorax = qobject_cast<QComboBox *>(editor);
            model->setData(index,editorax->currentText());
            //////////qDebug() << "### setModelData -> " << editorax->currentText();
         
     } else {
             QLineEdit *editorax = qobject_cast<QLineEdit *>(editor);
             model->setData(index,editorax->text());
            //////// qDebug() << "### setModelData -> " << editorax->text();
     }
 }


    
void BooksDelegate::commitAndCloseEditor()
{
    ////////////qDebug() << "### scommitAndCloseEditor -> ";
...
void BooksDelegate::emitCommitData()
{
