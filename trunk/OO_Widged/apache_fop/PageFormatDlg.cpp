	...
	: QDialog( parent )
{
    ...
    connect(lineEdit_2, SIGNAL(editingFinished()), this, SLOT(recalcAll()));
    connect(lineEdit_3, SIGNAL(editingFinished()), this, SLOT(recalcAll()));
    connect(lineEdit, SIGNAL(editingFinished()), this, SLOT(recalcAll()));
...
void PageFormatDlg::recalcAll()
{
    ...
    if (lineEdit_2->text().size() > 2) {
     currentP.doctitle = lineEdit_2->text();
    }
    if (lineEdit_3->text().size() > 2 ) {
     currentP.autor = lineEdit_3->text();
    }
    ...
    /*
     ...
     lineEdit_2->setText( e.doctitle );
     lineEdit_3->setText( e.autor );
