...
StyleParser::StyleParser( const Document *document, const QDomDocument &domDocument, StyleInformation *styleInformation )
  : mDocument( document ), mDomDocument( domDocument ),LogicalDPI_Y(DPIactualDiff()),
    ...
    if ( element.tagName() == QLatin1String( "meta" ) ) {
      ...
      while ( !child.isNull() ) {
        if ( child.tagName() == QLatin1String( "generator" ) ) {
          mStyleInformation->addMetaInformation( "producer", child.text(), QString( "Producer" ) );
        } else if ( child.tagName() == QLatin1String( "creation-date" ) ) {
          const QDateTime dateTime = QDateTime::fromString( child.text(), Qt::ISODate );
          /////////mStyleInformation->addMetaInformation( "creationDate", QLocale::locale()->formatDateTime( dateTime, QLocale::LongDate, true ),QString( "Created" ) );
        } else if ( child.tagName() == QLatin1String( "initial-creator" ) ) {
          mStyleInformation->addMetaInformation( "creator", child.text(), QString( "Creator" ) );
        } else if ( child.tagName() == QLatin1String( "creator" ) ) {
          mStyleInformation->addMetaInformation( "author", child.text(), QString( "Author" ) );
        } else if ( child.tagName() == QLatin1String( "date" ) ) {
          const QDateTime dateTime = QDateTime::fromString( child.text(), Qt::ISODate );
          ///////mStyleInformation->addMetaInformation( "modificationDate", QLocale::locale()->formatDateTime( dateTime, QLocale::LongDate, true ),QString( "Modified" ) );
        }
...
double StyleParser::convertUnit( const QString &data )
{
  ...
  #define INCH_TO_POINT(inch) ((inch)*72.0)    /* dpi table default 72 */
