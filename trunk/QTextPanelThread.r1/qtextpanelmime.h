...
class MountPage
{
    ...
    /* stream only */
    PanelPageSize model;
    QMap<int,RichDoc> absoluteLayer;  /* header footer and floating layer 
    order by zindex */
    RichDoc mainDoc;  /* flat auto floating text and is image */
    /* stream only */
    QImage cacheimg;  /* all page cache paint to view */
    MountPage() {}
    MountPage& operator=(const MountPage& d) {
    mainDoc = d.mainDoc;
    cacheimg = d.cacheimg;
    model = d.model;
    absoluteLayer = d.absoluteLayer;
    return *this;
    }
    operator QVariant() const {
    return QVariant::fromValue(*this);
    ...
    QTextDocument *doc() {
    return mainDoc.todoc();
    }
    
};

Q_DECLARE_METATYPE(MountPage);
