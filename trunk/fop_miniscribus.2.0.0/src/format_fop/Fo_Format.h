...
static inline  QTextBlockFormat PageBreackBlock( QTextBlockFormat rootformats = QTextBlockFormat() )
{
    ...
    rootformats.setLeftMargin(0);
    rootformats.setAlignment( Qt::AlignLeft );
    rootformats.setPageBreakPolicy(QTextFormat::PageBreak_AlwaysAfter);
    return rootformats;
}
...
typedef struct {
  QString xslt_file;
  QString xml_file;
  QString render_dir; /* image and xx ?? */
  QString result_file;
} FO_TRANSFORMER;
...
public:
    ...
    /* dom 2 qt */
    FoRegion regionFormat(const QDomElement e);
    ...
    QColor ColorFromFoString( QString focolor , FopColor::AlternateColor col = FopColor::Transparent );
