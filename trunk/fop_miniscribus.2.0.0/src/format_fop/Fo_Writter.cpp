...
QDomDocument FopDom::absoluteLayer( AbsoluteLayer *e)
{
    dom.clear();
    ...
    QDomElement lbock = dom.createElement("fo:block-container");
    
    bool dirpaint = false;
    M_PageSize Page = e->PageFormat; 
    FoRegion regio;
    regio.toAll(0);
    const qreal bloc_height = e->boundingRect().height();
    
        if (e->blockType() != DIV_ABSOLUTE) {
            if (e->blockType() == DIV_START) {
                dirpaint = Page.area[2].enable;
                regio = Page.area[2];
            } else if (e->blockType() == DIV_END) {
                dirpaint = Page.area[4].enable;
                regio = Page.area[4];
            } else if (e->blockType() == DIV_HEADER) {
                dirpaint = Page.area[0].enable; 
                regio = Page.area[0];
            } else if (e->blockType() == DIV_FOOTER) {
                dirpaint = Page.area[1].enable; 
                regio = Page.area[1];
            }
         }
         
    if (dirpaint) {
    regio.styleContainer(lbock,bloc_height);
    dom.appendChild( lbock );
    } else if ( e->blockType() == DIV_ABSOLUTE ) {
         QMapIterator <QString,QVariant> i(e->layerStyle());
         while (i.hasNext()) {
             i.next();
             if ( !i.value().toString().isEmpty() ) {
             lbock.setAttribute(i.key(),i.value().toString());
             }
         }
       /* check size ? */
       dom.appendChild( lbock );
  ...
  FrameLoop(doc->rootFrame()->begin(),lbock);
  return dom;  
}
...
void FopDom::SetDoc( QTextDocument * docin , M_PageSize page , LEVEL e /*FOP_APACHE*/  )
{
    ...
    sx->saveImageFile.clear();  /* clear image  list comming here */
  ...
  dom.appendChild( basexslforoot );
    ...
    /* append bookmark if having */
    if (sx->bokMarkDom.size() > 0) {
       QDomElement bbo = bookMarkTree( sx->bokMarkDom );
        if (bbo.tagName() == "fo:bookmark-tree") {
         basexslforoot.appendChild(bbo);
        }
    }
    ...
    pdftitle.appendChild( dom.createTextNode ( page.doctitle )  );
    pageseq1.appendChild( pdftitle );
    
    /* header*/
    if (page.area[0].enable) {
    ...
    page.area[0].styleContainer(a_txt_1,page.body.margin_top);
    ...
    page.area[1].styleContainer(a_txt_2,page.body.margin_bottom);
    ...
    page.area[2].styleContainer(a_txt_3,page.body.margin_left);
    ...
    page.area[3].styleContainer(a_txt_4,page.body.margin_bottom);
    ...
    qDebug() << "### books size   " << sx->bokMarkDom.size();
...
void FopDom::HandleTable( QTextTable  * childTable , QDomElement appender ) 
{
	...
	if (tbforms.background().color().name() !="#000000") {
        if (ColorValid(tbforms.background())) {
	...
	if (tbforms.background().color().name() !="#000000" && ColorValid(tbforms.background()) ) {
  ...
  const int rowline = childTable->rows();
	...
	tbody.appendChild(rows);
		              for (int ttd = 0; ttd < coolsums; ++ttd) {
										...
										if (cell.format().background().color().name() !="#000000" && ColorValid(cell.format().background())) {
...
void FopDom::HandleBlock( QTextBlock  para  , QDomElement appender ) 
{
    ...
    QTextBlockFormat blfo = para.blockFormat();
    const QString idmemo = blfo.property(BookMarkInternalID).toString();
        ...
        if (!idmemo.isEmpty()) {
            ...
            if (!sx->validKey(idmemo)) {
            /* is already register as unique */
            paragraph.setAttribute("id",idmemo); 
            }   
        } 
							      ...
							      ////////////////imagen.setAttribute ("id",Imagename(hrefadress)+"-"+Ivariant);
										if (!xx.isNull()) {
											  SPics pico = xx.value<SPics>();
                                               /* write correct sroll attributes !! */
...
void FopDom::PaintFopBlockFormat( QDomElement e , QTextBlockFormat bf )
{
      ...
      if (bf.background().color().name() !="#000000" && ColorValid(bf.background())   ) {
      e.setAttribute ("background-color",ColorFopString(bf.background()));
      }
      ...
      if (!idMemobockMarks.isEmpty()) {
        e.setAttribute ("id",idMemobockMarks);  
      }
...
bool FopDom::ColorValid( const QBrush paintcolor ) const 
{
	  ///////QColor::QColor ( int r, int g, int b, int a = 255 )
	  QColor c = paintcolor.color();
      return c.alpha() < 9 ? false : true;
}

bool FopDom::ColorValid( const QColor c ) const 
{
	  return c.alpha() < 9 ? false : true;
}
